...
class QueryGenerator {
  ...
  static alterIdentify(table, column, prevProperties, properties) {
    ...
    let options = SequenceQueryGenerator.setAttributes(properties)
      .map((query) => `set ${query}`)
      .join(' ');

    if (properties.generation) {
      options = `set generated ${properties.generation} ${options}`;
    }

    return `alter table ${table} alter column ${quotedColumnName} ${options};`;
  }
