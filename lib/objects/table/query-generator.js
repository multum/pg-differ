...
class QueryGenerator {
  ...
  getRestartSequenceValueByColumnData(column, sequence, currentValue) {
  ...
  alterIdentify(column, diff) {
    if (utils.isEmpty(diff)) return;
    const table = this.table.getQuotedObjectName();
    column = helpers.addQuotes(column.name);

    let options = SequenceQueryGenerator.setAttributes(diff);
    options = options.map((query) => `set ${query}`);

    if (utils.isExist(diff.generation)) {
      options.unshift(`set generated ${diff.generation}`);
    }

    if (utils.isExist(diff.restart)) {
      options.push(`restart with ${diff.restart}`);
    }

    return `alter table ${table} alter column ${column} ${options.join(' ')};`;
  }
