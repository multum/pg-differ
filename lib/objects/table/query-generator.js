...
const _getColumnDescription = (column, temp) => {
  const chunks = [`${helpers.addQuotes(column.name)} ${column.type.raw}`];
  ...
  if (column.identity) {
    chunks.push(_identityDescription(column.type.raw, column.identity));
  }
...
class QueryGenerator {
  ...
  getMaxValueForRestartSequence(column, sequence) {
    ...
    const defaults = Sequences.getDefaults(column.type.raw);
  ...
  createIndex(index) {
    ...
    return `create index on ${table} ( ${columns} );`;
  }
  ...
  createConstraint({
    ...
    const prefix = `alter table ${table} add ${
      name ? `constraint ${name} ${type}` : type
    }`;

    switch (type) {
      case 'unique':
      case 'primary key':
        return `${prefix} ( ${_joinColumns(columns)} );`;
      ...
      case 'foreign key': {
        ...
        columns = _joinColumns(columns);
        const refTable = helpers.quoteObjectName(
          references.table,
          this.table._getSchemaName()
        );
        const refColumns = _joinColumns(references.columns);
        const events = `on update ${onUpdate} on delete ${onDelete}`;
        return (
          prefix +
          ` ( ${columns} ) references ${refTable} ( ${refColumns} )${match} ${events};`
        );
      }
    }
  }
  ...
  addIdentify(column, properties) {
    ...
    return (
      ...
      ` add ${_identityDescription(column.type.raw, properties)};`
    );
  }
          ...
          if (key === 'name') {
            ...
            if (
              ...
              const collate = column.collate
                ...
                : '';
              return `alter table ${table} alter column ${name} type ${column.type.raw}${collate};`;
            } else {
