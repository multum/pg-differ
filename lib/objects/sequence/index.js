...
class Sequence extends AbstractObject {
  static validateRangeUpdate(prev, next) {
    if (utils.isExist(next.min) && BigInt(next.min) > BigInt(prev.min)) {
      throw new SynchronizationError(`You cannot increase 'min' value`);
    }
    if (utils.isExist(next.max) && BigInt(next.max) < BigInt(prev.max)) {
      throw new SynchronizationError(`You cannot decrease 'max' value`);
    }
  }
