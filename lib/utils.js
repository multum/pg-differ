/**
 * Copyright (c) 2018-present Andrew Vereshchak
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */
'use strict';

const util = require('util');

exports.isExist = value => value !== undefined && value !== null;

exports.findByName = (array, name, formerNames) => {
  return array.find(el => {
    if (el.name === name) {
      return true;
    } else if (formerNames) {
      return formerNames.includes(el.name);
    }
    return false;
  });
};

exports.findByName = (array, name, formerNames) =>
  array.find(el => {
    if (el.name === name) {
      return true;
    } else if (formerNames) {
      return formerNames.includes(el.name);
    }
    return false;
  });

exports.getDiff = (object, exclude) => {
  return Object.keys(object).reduce((acc, key) => {
    const leftValue = object[key];
    const rightValue = exclude[key];
    if (String(leftValue) !== String(rightValue)) {
      acc[key] = leftValue;
    }
    return acc;
  }, {});
};

exports.getCaller = () => {
  const traceFn = Error.prepareStackTrace;
  Error.prepareStackTrace = (err, stack) => stack;
  const stack = new Error().stack;
  Error.prepareStackTrace = traceFn;
  return stack[2].getFileName();
};

exports.isObject = target => {
  return target
    ? Object.prototype.toString.call(target) === '[object Object]'
    : false;
};

exports.pick = (keys, object) => {
  return keys.reduce((acc, key) => {
    if (Object.prototype.hasOwnProperty.call(object, key)) {
      acc[key] = object[key];
    }
    return acc;
  }, {});
};

exports.get = (path, obj) => {
  if (typeof path === 'string') {
    path = path
      .replace(/.?\[/g, match => (match === '[' ? '' : '.'))
      .replace(/]/g, '')
      .split('.');
  }

  for (const level of path) {
    obj = obj[level];
    if (obj === undefined) {
      break;
    }
  }

  return obj;
};

exports.findWhereEq = (arrayOfObjects, props) => {
  return arrayOfObjects.find(object => {
    const compareKeys = Object.keys(props);
    object = exports.pick(compareKeys, object);
    return util.isDeepStrictEqual(object, props);
  });
};

exports.isEmpty = target => {
  if (exports.isObject(target)) {
    target = Object.keys(target);
  }
  return target.length === 0;
};

exports.unnest = array => {
  return array.reduce((acc, i) => {
    return acc.concat(i);
  });
};

exports.once = fn => {
  let result;
  let called = false;
  return function() {
    if (called) return result;
    called = true;
    result = fn.apply(this, arguments);
    return result;
  };
};

exports.cutFromString = (regExp, target) => {
  let result;
  const replaced = target.match(regExp);
  if (replaced) {
    result = target.replace(regExp, '');
  } else {
    result = target;
  }
  return [result.trim(), replaced];
};
