...
describe('reading structure', function() {
  ...
  it('table', async function() {
    ...
    if (await differ._supportSeeds()) {
      ...
      const seedExpect = (number, properties) =>
        expect(users.seeds[number - 1]).to.deep.include(properties);
  ...
  it('undefined table', async function() {
    const undefinedTable = await differ.read.table({
      name: 'public.undefined_table',
    });
